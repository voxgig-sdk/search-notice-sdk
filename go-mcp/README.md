...
[MCP](https://modelcontextprotocol.io) server exposing the SearchNotice SDK as
two agent tools — `search-notice_list` and `search-notice_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/search-notice-mcp)
make build

# 2. Provide credentials via the environment
export SEARCH_NOTICE_APIKEY=sk_live_xxx
...
```jsonc
// search-notice_list: first page of records
{ "entity": "search" }
{ "entity": "search", "query": { } }

// search-notice_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/search-notice-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user search-notice \
     -- "$PWD"/dist/*/search-notice-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `search-notice_list` and `search-notice_load` tools now appear
   in new sessions. Ask the agent to *"list search using search-notice"*
   and it calls `search-notice_list` with `{"entity":"search"}`.
...
```sh
export SEARCH_NOTICE_APIKEY=sk_live_xxx            # API key
export SEARCH_NOTICE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `search-notice_list` tool
...
### Call the `search-notice_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `search-notice_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `search-notice_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SEARCH_NOTICE_APIKEY` | API key sent with every request. |
| `SEARCH_NOTICE_BASE` | Optional override of the API base URL. |
