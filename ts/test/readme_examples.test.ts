...
//     (\`new SearchNoticeSDK(...)\` / \`SearchNoticeSDK.test(...)\` become
//     \`SearchNoticeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'SearchNoticeSDK'
...
// rewrite real client construction (\`new SearchNoticeSDK(...)\` and any
// \`SearchNoticeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SearchNoticeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SearchNoticeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
