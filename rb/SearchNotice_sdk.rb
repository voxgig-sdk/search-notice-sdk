...
require_relative 'SearchNotice_types'


class SearchNoticeSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = SearchNoticeHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = SearchNoticeHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, SearchNoticeFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    SearchNoticeUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = SearchNoticeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = SearchNoticeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = SearchNoticeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => SearchNoticeError.new(
        "#{op}_allow",
        "SearchNoticeSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = SearchNoticeHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = SearchNoticeError.new(
        "graphql_error", "SearchNoticeSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = SearchNoticeSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
