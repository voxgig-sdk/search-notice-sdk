# SearchNotice SDK Search entity
...
class SearchEntity
  ...
  def data_set(args)
    if args
      @_data = SearchNoticeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = SearchNoticeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Search, Hash] the loaded Search; raises SearchNoticeError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = SearchNoticeHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
