...
module SearchNoticeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SearchNoticeResponse.new({})
    ...
    if fetch_err
      ...
      response = SearchNoticeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SearchNoticeResponse.new(fetched)
    else
