...
// SearchNotice SDK retry feature
...
class SearchNoticeRetryFeature extends SearchNoticeBaseFeature
{
    ...
    public function init(SearchNoticeContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SearchNoticeContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SearchNoticeContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SearchNoticeContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
