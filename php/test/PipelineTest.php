...
// SearchNotice SDK pipeline test
//
...
{
    private static function utility(): SearchNoticeUtility
    {
        return new SearchNoticeUtility();
    }
    ...
    private static function ctx(array $over = []): SearchNoticeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new SearchNoticeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new SearchNoticeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof SearchNoticeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new SearchNoticeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = SearchNoticeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new SearchNoticeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = SearchNoticeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new SearchNoticeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = SearchNoticeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SearchNoticeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = SearchNoticeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new SearchNoticeResponse([]),
            'result' => new SearchNoticeResult([]),
        ]);
        [, $err] = SearchNoticeMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new SearchNoticeSpec([]),
            'response' => null,
            'result' => new SearchNoticeResult([]),
        ]);
        [, $err] = SearchNoticeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new SearchNoticeSpec(['step' => 's']),
            'response' => new SearchNoticeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new SearchNoticeResult([]),
        ]);
        [, $err] = SearchNoticeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new SearchNoticeSpec(['step' => 's']),
            'response' => new SearchNoticeResponse(self::resp(200, ['v' => 1])),
            'result' => new SearchNoticeResult([]),
        ]);
        [, $err] = SearchNoticeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new SearchNoticeSpec(['step' => 's']),
            'response' => new SearchNoticeResponse(self::resp(200, ['v' => 2])),
            'result' => new SearchNoticeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        SearchNoticeMakeResponse::call($ctx);
    ...
    {
        $preset = new SearchNoticeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new SearchNoticeSpec([]),
            'response' => new SearchNoticeResponse([]),
            'result' => new SearchNoticeResult([]),
        ...
        [$got, $err] = SearchNoticeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new SearchNoticeResult([])]);
        [, $err] = SearchNoticeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new SearchNoticeSpec([]), 'result' => null]);
        [, $err] = SearchNoticeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SearchNoticeSpec(['step' => 's']),
            'result' => new SearchNoticeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new SearchNoticeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SearchNoticeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SearchNoticeSpec(['step' => 's']),
            'result' => new SearchNoticeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new SearchNoticeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SearchNoticeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SearchNoticeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new SearchNoticeSpec([]),
            'result' => new SearchNoticeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = SearchNoticeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = SearchNoticeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new SearchNoticeError('boom', 'boom');
        $utility->fetcher = function (SearchNoticeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SearchNoticeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = SearchNoticeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (SearchNoticeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = SearchNoticeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(SearchNoticeResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        SearchNoticeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (SearchNoticeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SearchNoticeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = SearchNoticeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new SearchNoticeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new SearchNoticeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = SearchNoticeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = SearchNoticeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = SearchNoticeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new SearchNoticeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, SearchNoticeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new SearchNoticeResult(['ok' => false])]);
        $this->expectException(SearchNoticeError::class);
        SearchNoticeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new SearchNoticeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', SearchNoticeMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        SearchNoticeMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new SearchNoticeBaseFeature();
        $b = new SearchNoticeBaseFeature();
        SearchNoticeFeatureAdd::call($ctx, $a);
        SearchNoticeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): SearchNoticeBaseFeature
    {
        $f = new SearchNoticeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        SearchNoticeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): SearchNoticeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new SearchNoticeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = SearchNoticePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        SearchNoticePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new SearchNoticeResponse(['headers' => null]),
            'result' => new SearchNoticeResult([]),
        ]);
        SearchNoticeResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        SearchNoticeResultBody::call($ctx);
