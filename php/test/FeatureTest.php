...
class FtCtrl extends SearchNoticeControl
{
...
class FtClient
{
    ...
    private SearchNoticeUtility $utility;

    public function __construct(SearchNoticeUtility $utility, array $options)
    {
    ...
    public function get_utility(): SearchNoticeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SearchNoticeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SearchNoticeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SearchNoticeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SearchNoticeConfig::make_feature($name);
    ...
    private static function buildUrl(SearchNoticeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SearchNoticeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SearchNoticeResponse($fetched);
        }
    ...
    private function fail(SearchNoticeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SearchNoticeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
