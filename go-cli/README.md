...
boru-driven command-line client **and** interactive REPL for the SearchNotice
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/search-notice-cli)
make build

# 2. See usage (words, entities, env vars)
./search-notice-cli --help

# 3. Provide credentials once, via the environment
export SEARCH_NOTICE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./search-notice-cli load 1 search            # {id:1} shorthand
./search-notice-cli load '{id:1}' search       # explicit match map

# 5. Override the API base URL for a single call
SEARCH_NOTICE_BASE=https://api.example.com ./search-notice-cli load 1 search

# 6. No arguments -> interactive REPL
./search-notice-cli
search-notice> load 1 search
search-notice> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/search-notice-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/search-notice-cli load 1 search
   ```
...
```sh
./search-notice-cli load 1 search          # scalar shorthand for {id:1}
./search-notice-cli load '{id:1}' search     # explicit match map
```
...
./search-notice-cli load 1 search
```
...
Run with no arguments to open a REPL (prompt `search-notice>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/search-notice-cli`. |
...
(`./search-notice-cli <expr>`) and interactively (the REPL), and expressions compose the
