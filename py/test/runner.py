...
class SearchNoticeTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SearchNoticeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SearchNoticeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SearchNoticeTestRunner.getenv("SEARCH_NOTICE_TEST_LIVE")
        override = SearchNoticeTestRunner.getenv("SEARCH_NOTICE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SearchNoticeTestRunner.getenv(key)
        ...
        explain = SearchNoticeTestRunner.getenv("SEARCH_NOTICE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if SearchNoticeTestRunner._test_control is not None:
            return SearchNoticeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SearchNoticeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SearchNoticeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SearchNoticeTestRunner.load_test_control()
...
def load_env_local():
    SearchNoticeTestRunner.load_env_local()


def env_override(m):
    return SearchNoticeTestRunner.env_override(m)


def entity_data(v):
    return SearchNoticeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SearchNoticeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SearchNoticeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SearchNoticeTestRunner.load_test_control()


def live_delay_ms():
    return SearchNoticeTestRunner.live_delay_ms()
