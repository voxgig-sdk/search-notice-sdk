...
func TestSearchEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SEARCHNOTICE_TEST_SEARCH_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func searchBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SEARCHNOTICE_TEST_SEARCH_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"SEARCHNOTICE_TEST_SEARCH_ENTID": idmap,
		"SEARCHNOTICE_TEST_LIVE":      "FALSE",
		"SEARCHNOTICE_TEST_EXPLAIN":   "FALSE",
	})

	idmapResolved := core.ToMapAny(env["SEARCHNOTICE_TEST_SEARCH_ENTID"])
	if idmapResolved == nil {
		idmapResolved = core.ToMapAny(idmap)
	}

	if env["SEARCHNOTICE_TEST_LIVE"] == "TRUE" {
		mergedOpts := vs.Merge([]any{
			map[string]any{
			},
			extra,
		})
		client = sdk.NewSearchNoticeSDK(core.ToMapAny(mergedOpts))
	}
	...
	return &entityTestSetup{
		...
		idmap:         idmapResolved,
		env:           env,
		explain:       env["SEARCHNOTICE_TEST_EXPLAIN"] == "TRUE",
		live:          live,
